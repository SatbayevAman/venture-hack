...
| Шаг | Где | Что |
|---|---|---|
| B1 | `core/knowledge.py` | Классическая BKT (Corbett & Anderson, 1994): `BKTParams(L0, T, S, G)`, `bkt_step`, `bkt_trajectory(seq, params)`. `fit_bkt(seqs)` перебирает сетку L0 0.1–0.9 (шаг 0.1), T 0.05–0.4, S 0.05–0.3, G 0.1–0.4 (шаг 0.05), всего 3024 точки, векторно на numpy, и берёт максимум правдоподобия. Меньше 10 наблюдений — значения по умолчанию `0.3 / 0.15 / 0.1 / 0.2`. `fit_for(conn, skill, tag)` подбирает параметры по всему классу, результат кэшируется по самим последовательностям. `mastery(...)` — траектория ученика. В интерфейсе: «Вероятность, что навык освоен: 0.33 → 0.80 (после ДЗ №1 → №7)» и объяснение модели в подписи раздела. |
