...
| B4 | Темы Беты: правила для `ineq_flip`, `boundary` в `RULES`; шаблон неравенства регистрируется (`register_optional_topics`) только если тег есть в `T.TAGS`. Задача попадает в набор, лишь когда её эталон проходит `check_problem`. После слияния с Бетой (`kind="inequality"`) все эталоны проходят её проверку, пропущенная смена знака классифицируется как `ineq_flip`; прочие теги неравенств (`ineq_div_var` и др.) получают на ступени 3 пример-неравенство. `targets_for` предлагает только теги, у которых реально генерируются задачи. |
...
```
$ python -m pytest -q
300 passed in 62.6s         # после слияния main (все агенты); в tests/test_gamma.py — 64 теста
```
...
- **Неравенства:** шаблон один (`ineq_flip`, он же для `boundary`); отдельных шаблонов для `ineq_div_var`, выбора промежутка и систем пока нет.
...
- **Бета:** вид `inequality` и запись ответа совпали — тренажёр по `ineq_flip` работает на её проверке (тест `test_inequality_practice_with_beta_checker`). Правила для `ineq_div_var` и систем можно добавить в `hints.RULES`.
- **Дзета:** тренажёр берёт учеников только из `students()` и открыт роли «ученик» (`STUDENT_PAGES`); проверено входом демо-ученика — видна только Айгерим. Блок «Сводка по классу» ученику показывает лишь его строку, но его стоит скрыть для роли «ученик» (передать флаг роли в `render`).
