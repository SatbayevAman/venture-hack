...
- **`PORTRET_DEMO=0` только скрывает кнопки демо-входа.** Демо-аккаунты создаются всё равно; если задан `PORTRET_DEMO_PASSWORD`, в них можно войти формой (демо-учитель видит только синтетический 8 «А»). Для настоящей школы `PORTRET_DEMO_PASSWORD` не задавать. Замечено при сверке README; в план не входило, не менялось.
