...
**Итог:** `python -m pytest -q` → **400 passed** (334 прежних без правок + 66 в `tests/test_omega.py`, без сети и без ключа API). Демо «Потеря корня: 4/6 → 5/7» не изменилось; повторная проверка той же работы снова даёт 5/7. Один коммит на пункт, сначала P0.
...
| O8 | `core/ocr.py`, `core/kinds/*`, `core/llm.py`, `app.py`, `evaluate.py` | 1) `score_lines(lines, kind=None)` / `score_line(line, kind=None)`: если `kind` есть в `core.kinds.KINDS`, разбирается ли строка, решает `parses(raw)` модуля вида (реестр `kinds.PARSES`). `parses` написаны в `inequality`, `system`, `biquadratic` на их собственных разборщиках и повторяют ветки их `check`: `False` — там, где `check` ставит строке `unparsed`. У биквадратных разбор зависит от места строки (уравнение в t — целиком, строка в x после обратной замены — по левой части и корням), и `parses` без контекста следует этим правилам по виду самой строки. `app._prep` передаёт `p["kind"]`, `evaluate.py` — `kind` из разметки; `--kind` у `--draft` принимает и виды Беты. Без `kind` поведение прежнее. 2) `clean_text`: `\infty → ∞`, `\cup → ∪`, `\in → ∈`, `\{` и `\}` удаляются (заменяются пробелом, чтобы `\cup\{3\}` не склеилось в `\cup3`). Строки «без задачи», которые учитель отнёс к задаче, переоцениваются по её виду. 3) `llm._RULES`: знаки `< > ≤ ≥`, `∞`, `∪`, скобки промежутков и `;` — как в тетради; строка знаков «+ − +» — как есть; система — двумя строками, большую скобку не переписывать; запрет — только на фигурные скобки LaTeX. Контракт `recognize()` прежний. |
...
| O14 | `core/quality.py`, `evaluate.py`, `ui/review.py` | `quality.reference_accuracy(labels)` — одна функция: пропускает черновики `status=draft`, возвращает их число (`n_draft`) и строки сверки. Её вызывают `quality.eval_accuracy` (страница «Качество», слайд) и часть `evaluate.py` про эталонные строки. Страница «Качество» пишет, сколько черновиков пропущено, а если в разметке одни черновики — так и говорит (раньше: «нет строк»). |
...
- **O8, `parses` повторяет ветки `check`.** Разбор строки у неравенств и систем зависит от вида строки (промежуток, пара, строка системы), поэтому общий `is_unparsable` для них не годится. Согласованность `parses` с `check` проверяется тестами на эталонах и типичных ошибках ДЗ №8–10 и на строках биквадратного уравнения после замены в контексте; вручную сверено и на всех 59 параметризованных случаях `tests/test_beta.py` — расхождений нет. Для строк вне этих наборов совпадение не гарантировано (см. «Риски»).
...
```text
$ python -m pytest -q
400 passed                       # 334 прежних без правок + 66 новых
$ python evaluate.py
...
| `ui/review.py` | В eval/labels.csv только черновики ({n}): проверьте эталон по фото и уберите пометку draft — см. eval/README.md. | eval/labels.csv-те тек нобайлар ({n}): эталонды фото бойынша тексеріп, draft белгісін алып тастаңыз — eval/README.md қараңыз. |
| `ui/review.py` | Черновиков пропущено: {n}. | Өткізілген нобайлар: {n}. |
...
- **Найдено при финальной проверке, вне плана, не исправлялось** (были до правок Омеги; «Новых возможностей не добавляй» — исправлять отдельной задачей):
  - **`p["methods"]` считает методы всех навыков, а делит на работы с квадратными.** После живой проверки ДЗ №10 (биквадратное через дискриминант) в разделе «2. Как решает» — «Дискриминант — в 7 работах из 6». План (O11) велел `p["methods"]` не трогать; `methods_by_skill` считает верно (квадратные 6/6, биквадратные 1/1). Правка — считать в `methods` только наблюдения навыка `quadratic`; числа демо от этого не меняются.
  - **`check_problem("biquadratic", …)` падает с `TypeError`** на строке «y = x²» после замены через t (`biquadratic._in`): замена переменной буквой `y` не распознаётся. На экране проверки это исключение при «Проверить».
- **`parses` без контекста.** Для строк вне проверенных наборов совпадение с `check` не гарантировано: у биквадратных `check` разбирает строку по-разному в зависимости от её места (до замены, уравнение в t, после обратной замены).
- **`PORTRET_DEMO=0` только скрывает кнопки демо-входа.** Демо-аккаунты создаются всё равно; если при их создании был задан `PORTRET_DEMO_PASSWORD`, в них можно войти формой (демо-учитель видит только синтетический 8 «А»). Для настоящей школы `PORTRET_DEMO_PASSWORD` не задавать. Замечено при сверке README; в план не входило, не менялось.
