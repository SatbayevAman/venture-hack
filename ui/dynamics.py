...
def render_section(conn, p: dict, L, lang: str) -> None:
    ...
    st.caption(L("Работы от старых к новым. Интервал — 80 % по Уилсону: где, скорее всего, настоящая доля. "
                 ...
                 "она растёт, после работы с ошибкой — падает; параметры подобраны по данным всех учеников в базе. "
                 ...
                 "қатесі бар жұмыстан кейін төмендейді; параметрлер базадағы барлық оқушының деректері бойынша таңдалған. "
